...
class CoffeeShopGUI:
    ...
    def create_widgets(self):
        ...
        self.label = tk.Label(self.master, text="Welcome to Coffee Place!", font=("Arial", 16))
        self.label.pack(pady=10)

        self.menu_listbox = tk.Listbox(self.master, selectmode=tk.SINGLE, height=len(self.products), font=("Arial", 12), width=40)
        for item in self.cashier.display_menu():
            self.menu_listbox.insert(tk.END, item)
        self.menu_listbox.pack(pady=10)

        self.quantity_label = tk.Label(self.master, text="Quantity:", font=("Arial", 12))
        self.quantity_label.pack()

        self.quantity_entry = tk.Entry(self.master, font=("Arial", 12), width=10)
        self.quantity_entry.pack()

        self.add_button = tk.Button(self.master, text="Add to Order", command=self.add_to_order, font=("Arial", 12))
        self.add_button.pack(pady=10)

        self.order_label = tk.Label(self.master, text="Your Order:", font=("Arial", 14, "bold"))
        self.order_label.pack()

        self.order_listbox = tk.Listbox(self.master, selectmode=tk.SINGLE, height=5, font=("Arial", 12), width=40)
        self.order_listbox.pack()

        self.remove_button = tk.Button(self.master, text="Remove Item", command=self.remove_item, font=("Arial", 12))
        self.remove_button.pack(pady=10)

        self.complete_order_button = tk.Button(self.master, text="Complete Order", command=self.complete_order, font=("Arial", 12))
